...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        // Use redirectUrl if provided, otherwise use role-based redirect
        const targetUrl = 
          redirectUrl !== '/' 
            ? redirectUrl 
        ...
        // Use window.location.href instead of router.push() to ensure full page reload with cookies
        setTimeout(() => {
      ...
      setError(result.message || result.error || 'Invalid phone number or password')
    } catch (err: unknown) {
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div>}>
      <LoginForm />
    </Suspense>
  )
}
